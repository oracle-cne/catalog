...
This chart bootstraps an ingress-nginx deployment on a [Kubernetes](http://kubernetes.io) cluster.
...
Use `ocne application install --help` for command documentation.

## Uninstall Chart

```console
ocne application uninstall --release [RELEASE_NAME] --namespace ingress-nginx
```

This removes all the Kubernetes components associated with the chart and deletes the release.

Use `ocne application uninstall --help` for command documentation.

## Upgrading Chart

```console
ocne application update --release [RELEASE_NAME] --namespace ingress-nginx
```

Use `ocne application update --help` for command documentation.
...
Note that there are some different and upgraded configurations between the two charts, described by Rimas Mocevicius from JFrog in the "Upgrading to ingress-nginx Helm chart" section of [Migrating from Helm chart nginx-ingress to ingress-nginx](https://rimusz.net/migrating-to-ingress-nginx). As the `ingress-nginx/ingress-nginx` chart continues to update, you will want to check current differences by reviewing the [configuration](#configuration) for both charts.
...
A validating and configuration requires the endpoint to which the request is sent to use TLS. It is possible to set up custom certificates to do this, but in most cases, a self-signed certificate is enough. The setup of this component requires some more complex orchestration during installation and upgrades. The steps are created to be idempotent and to allow turning the feature on and off without running into release management quirks.
