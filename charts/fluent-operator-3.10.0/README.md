...
```shell
ocne application install --release [RELEASE_NAME] --name fluent-operator --namespace [NAMESPACE]
```
...
- CRDs are not upgraded by `ocne application update`; apply CRD updates manually
- CRDs are not deleted by `ocne application uninstall`

**Upgrading CRDs:**

When upgrading, manually apply CRD updates before upgrading the application. Obtain the CRDs from the repository:
...
**Major Changes:**
- CRDs now in the `crds/` directory
